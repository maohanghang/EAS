                            ...
                            <table class="table table-bordered table-striped js-dataTable-full">
                                <thead>
                                 ...
                                 <% 
                                    	...
                                    	List userAllstudent= aDAO.selectAllstudent();
                                    	System.out.println(userAllstudent.size());
                                    	request.setAttribute("userAllstudent", userAllstudent); 
                                  
                                     %>
                                <tbody>
                                <c:forEach items="${userAllstudent}" var="item" varStatus="status">  
								  <tr>
                                   
                                        <td class="text-center">${status.index+1}</td>
                                        <td class="font-w600">${item.getSno() }</td>
                                        <td class="font-w600">${item.getSname() }</td>
                                        <td class="hidden-xs">${item.getSsex() }</td>
                                        <td class="hidden-xs">${item.getSbirth() }</td>
                                        <td class="hidden-xs">${item.getClassno() }</td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" 
                                                title="Edit Client" onclick="edit('${item.getSno() }',
                                                                                  '${item.getSname() }',
                                                                                  '${item.getSsex() }',
                                                                                  '${item.getSbirth() }',
                                                                                  '${item.getClassno() }')">
                                                <i class="fa fa-pencil"></i></button>
                                                <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Remove Client" onclick="del('${item.getSno()}')"><i class="fa fa-times"></i></button>
                                            </div>
                                        </td>
                                    </tr> 
								</c:forEach>  
    ...
    <form action="${pageContext.request.contextPath}/UpdateStudent" method="post">
            ...
            <div class="modal-body">
            	...
            	<div class="form-group">
                    <input type="text" class="form-control" id="sno" placeholder="学号" name="sno" readonly="true">
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" id="sname" name="sname" placeholder="姓名">
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" id="ssex" name="ssex" placeholder="性别" >
                </div>
                
                <div class="form-group">
                    <input type="text" class="form-control" id="sbrith" name="sbrith" placeholder="生日" >
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" id="classno" name="classno" placeholder="班级" >
                </div>
...
<script type="text/javascript">
     ...
     *编辑方法
     **/
    function edit(sno,sname,ssex,sbrith,classno) {
        console.log(name);
        editFlag = true;
